/*  Copyright (c) David Anderson 2001, 2002 */
...
test_control_menu(GtkWidget *widget, gpointer data)
{
	...
	case (uint)TEST_KILL_ALL:       
		common_kill(KILL_ALL, MAX_IO_TESTS);
                file_cleanup();
		break;

	case (uint)TEST_MESSAGES_ALL:   
...
usex_control_menu(GtkWidget *widget, gpointer data)
{
	...
	case (uint)USEX_SNAPSHOT:   
		dump_status(INTERACTIVE_STATUS, NULL);
		break;

	case (uint)SAVE_INPUT_FILE: 
		Shm->mode |= SAVE_DEFAULT;
		/* FALLTHROUGH */

	case (uint)SHOW_INPUT_FILE:  
	        if (Shm->infile && 
